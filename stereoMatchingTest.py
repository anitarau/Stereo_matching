...
#logs_path = Dir + '/tensorflow_workstation/anita/7_layer'
MODEL_FILENAME = Dir+"/tensorflow_workstation/anita/7_layer_tv/kitti_7layer_2_300000"
#MODEL_FILENAME = Dir+"/tensorflow_workstation/anita/9_layer_3pool/kitti_9layer_3pool_600000"

#MODEL_FILENAME = Dir+"/tensorflow_workstation/anita/7_layer_nopooling/7layer_nopooling_4"
#MODEL_FILENAME = Dir+"/tensorflow_workstation/anita/9_layer_tv/kitti_9layer_2_300000"
""" ---------- Helper functions ------------- """
...
def load_kitti():
    ...
    return left_images_validation, right_images_validation, disp_images_validation
...
def test_model():
    ...
    left_images, right_images, disp_images = load_kitti()
    ...
    for j in range(40):
        ...
        gt = disp_images[j][...,np.newaxis]
        left = left_images[j][np.newaxis,...]
        right = right_images[j][np.newaxis,...]

        gt_resized = tf.image.resize_images(gt,resize_shape,method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
        left_resized = tf.image.resize_images(left,resize_shape)
        right_resized = tf.image.resize_images(right, resize_shape)
        #print(left_resized)
        gt_,left_,right_ = sess.run([gt_resized,left_resized,right_resized])
        #print('test')
        ...
        h9_left_,h9_right_ = sess.run([h9_left,h9_right], feed_dict={x_left:left_,
                                          ...
                                          phase:False})
    #print(h9_left_)
    #test = inner_product_test(h9_left_,h9_right_,4)
        test = inner_product_test(h9_left_,h9_right_,batch_size,376,1240,129)
    #print(test)
    ...
    #print(output_layer_t_.shape())
        ...
        #plt.imshow(im_gt[0,:,:])
        #plt.show()
        #print('training: ',np.argmax(test,3)[0,250:270,500:520], 'true: ', im_gt[0,250:270,500:520])
        #if j ==16:
        #    tstim = np.argmax(test,3)[0,:,:]
        #    tstidx = im_gt[0,:,:]==0
        #    tstim[tstidx] = 0
        ...
        #    plt.imshow(im_gt[0,:,:])
        #    plt.show()
            #np.save('tensorflow_workstation/anita/evals/fig11_nopool',tstim)
            #np.save('tensorflow_workstation/anita/evals/fig11_true',im_gt[0,:,:])
        #print('training: ',np.argmax(test,3)[0,250:270,500:520], 'true: ', im_gt[0,250:270,500:520])
    
        print('testing error: ', error_val_1)
        total_error += error_val_1
    print('average test error: ',total_error/40)
